...
pub trait TargetScheduler<'a> {
    ...
    fn target(&'a mut self, target: &String);
}

pub struct LazyTargetSchedler<'a> {
    all_targets: Arc<AHashMap<&'a str, &'a Target<'a>>>,
...
impl<'a> LazyTargetSchedler<'a> {
    fn resolve_target(
        ...
        for item in deps {
            unresolved_targets.push(self.all_targets.get(item).unwrap());
        }
    }
    fn get_runable_target(
        &self,
        todo_targets: &MutexGuard<Vec<&'a Target<'a>>>,
        done_targets: &MutexGuard<AHashSet<&'a str>>,
    ) -> Option<&'a Target<'a>> {
    ...
    pub fn new(all_targets: Arc<AHashMap<&'a str, &'a Target<'a>>>) -> LazyTargetSchedler<'a> {
        LazyTargetSchedler {
            all_targets,
...
impl<'a> TargetScheduler<'a> for LazyTargetSchedler<'a> {
    ...
    fn target(&mut self, target: &String) {
        let t = self.all_targets.get(target.as_str());
        self.unresolved_targets.lock().push(t.unwrap());
    }
}
